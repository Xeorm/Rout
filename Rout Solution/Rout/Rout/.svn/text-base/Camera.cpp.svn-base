...
{
	//mPosition += d*mRight;
	mPosition += D3DXVECTOR3(d*mRight.x, 0.0f, d*mRight.z);
}

void Camera::walk(float d)
{
	//mPosition += d*mLook;
	mPosition += D3DXVECTOR3(d*mLook.x, 0.0f, d*mLook.z);
}

void Camera::pitch(float angle)
{
	D3DXMATRIX R;
	if(currentPitch + angle > PI * 4 / 9)
	{
		angle = PI * 4 / 9 - currentPitch;
	}
	else if(currentPitch + angle < -PI * 4 / 9)
	{
		angle = -PI * 4 / 9 - currentPitch;
	}
