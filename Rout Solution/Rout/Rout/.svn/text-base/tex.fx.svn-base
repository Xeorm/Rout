...
float4 PS(VS_OUT pIn) : SV_Target
{
	...
	{
			if( gLightTypes[gActive[i]] == 0 ) // Parallel
			{
				litColor += ParallelLight(v, gLight[gActive[i]], gEyePosW);
			}
			else if( gLightTypes[gActive[i]] == 1 ) // Point
			{
				litColor += PointLight(v, gLight[gActive[i]], gEyePosW);
			}
			else // Spot
			{
				litColor += Spotlight(v, gLight[gActive[i]], gEyePosW);
			}
	}
