...
FullTile::FullTile(void)
{
}

FullTile::FullTile(D3DXVECTOR3 pos)
{
	type = "FullTile";
	ge3D_position = pos;
	collider = new Collider(8.0f, 8.0f, 8.0f, D3DXVECTOR3(pos.x - 4, pos.y - 4, pos.z - 4));
}

Collider* FullTile::getCollider(void)
{
	return collider;
}
...
void FullTile::interact(Player* player)
{
	//player->setPos(player->getLastPos());
	player->bounce(ge3D_position);
}

void FullTile::init(ID3D10Device* device)
{
	md3dDevice = device;
