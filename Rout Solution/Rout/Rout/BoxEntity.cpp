...
BoxEntity::BoxEntity(D3DXVECTOR3 pos, float width, float height, float depth)
{
	type = "BoxEntity";
	ge3D_position = pos;
	collider = new Collider(width, height, depth, D3DXVECTOR3(pos.x - width / 2, pos.y - height / 2, pos.z - depth / 2));
	Visible = true;
}
...
Collider* BoxEntity::getCollider(void)
{
	return collider;
}

void BoxEntity::init(ID3D10Device* device)
{
	md3dDevice = device;
